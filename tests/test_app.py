...
@pytest.mark.order(2)
async def test_add_data_to_pg(ac: AsyncClient, mock_db):
    with open(test_csv_pg, "rb") as fp:
        response = await ac.post("/create_and_insert_pg", files={"data": fp})
    # test response
    assert response.status_code == 200, response.json()
    assert response.json()["message"] == "ok", response.json()
    # test load to postgresql
    pg_rows = mock_db.count_rows()
...
async def test_find_simular_images_invalid_format(ac: AsyncClient):
    ...
    with open(file_path, "rb") as f:
        response = await ac.post("/find_simular_images", files={"image": f})

    assert response.status_code == 400
    assert "invalid image format" in response.text
    os.remove(file_path)
...
async def test_find_simular_images_valid_format(ac: AsyncClient):
    ...
    if response.status_code == 500:
        ...
        condidat_list = []
        for para in htmlParse.find_all("p", class_="fw-bold text-center text-muted"):
            condidat_list.append(para.get_text().replace("\\", "").replace("\n", ""))
